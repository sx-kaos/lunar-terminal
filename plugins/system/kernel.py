...
"""
Windows command/WinAPI interface for Cyber Sec Terminal.
...
class Kernel:
    ...
    def enable_privilege(name: str) -> tuple[bool, str]:
        """
        Enable a process-token privilege (e.g. 'SeDebugPrivilege').
        Returns (success, message). Requires elevation for most privileges.
        """
        if os.name != "nt":
        ...
        if not advapi32.OpenProcessToken(kernel32.GetCurrentProcess(),
            ...
            return False, f"OpenProcessToken failed: {Kernel.get_last_error_message(err)}"

        try:
            luid = LUID()
            if not advapi32.LookupPrivilegeValueW(None, name, ctypes.byref(luid)):
                err = ctypes.get_last_error()
                return False, f"LookupPrivilegeValue failed: {Kernel.get_last_error_message(err)}"
            ...
            if not advapi32.AdjustTokenPrivileges(token, False, ctypes.byref(tp), 0, None, None):
                err = ctypes.get_last_error()
                return False, f"AdjustTokenPrivileges failed: {Kernel.get_last_error_message(err)}"

            # Even if AdjustTokenPrivileges returns TRUE, check GetLastError
            err = ctypes.get_last_error()
            if err == ERROR_NOT_ALL_ASSIGNED:
                return False, "Privilege not held by the token (run as Administrator)."

            return True, "Privilege enabled."
        finally:
