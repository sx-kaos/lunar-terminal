...
def base_workspace_root() -> Path:
    ...
    Base directory where individual secure workspaces live:
      - Windows: %LOCALAPPDATA%/CyberSecTerminal/workspace
      - POSIX:   ~/.cybersec/workspace
    ...
    if os.name == "nt":
        ...
        root = base_dir / "CyberSecTerminal" / "workspace"
        root.mkdir(parents=True, exist_ok=True)
        _windows_hide(root)
        return root

    root = Path.home() / ".cybersec" / "workspace"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _harden_acls(path: Path) -> None:
    ...
    cmds = [
        ...
        ["icacls", str(path), "/grant", f"SYSTEM:(OI)(CI)(F)"],
...
def create_secure_workspace(
    ...
    """
    Create a new secure subfolder under the base workspace root, hide it,
    chdir into it, and record it in the database.
    """
...
def switch_secure_workspace(identifier: str) -> Path:
    """
    Switch to an existing workspace by id/slug/nickname (supports short-id prefix).
    """
    # LAZY IMPORT to avoid circular dependency
    ...
    if not path.exists():
        # Auto-archive stale catalog entries
        archive_workspace(row["id"])
        raise FileNotFoundError(
            f"Workspace path missing on disk and was archived: {path}"
        )

    # For existing workspaces, don't re-encrypt/reset ACLs blindly.
    # Still isolate TEMP so subprocesses spill into this workspace.
    _isolate_temp(path)
...
def resolve_in_sandbox(user_supplied_path: str | os.PathLike[str]) -> Path:
    ...
    resolved_path = (base_dir / path_obj if base_dir !=
                     Path("/") else path_obj).resolve()
