...
def suggest(text_before_cursor: str) -> list[str]:
    ...
    if key is not None:
        ...
        if not provider:
            return []
        return [f"{key}={value}" for value in provider(text=value_prefix, argv=argument_tokens, index=None)]
    ...
    if not provider:
        return []
    return list(provider(text=current_token, argv=argument_tokens, index=position_index))
